<!DOCTYPE html>
<html lang="en">

<head>
    <meta name="viewport" content="width=device-width,initial-scale=1.0">
    <meta property="og:title" content="KCET - Online Event Registration 2020-2021" />
    <meta property="og:description" content="Online Events started for 2020-2021 at KCET" />
    <meta property="og:url" content="http://www.kamarajengg.edu.in/Events/index.php" />
    <meta property="og:image" content="http://www.kamarajengg.edu.in/Events/img/king.jpg" />
    
    <!-- Title Page-->
    <title>Courses Page</title>
    <!-- Icons font CSS-->
    <link href="vendor/mdi-font/css/material-design-iconic-font.min.css" rel="stylesheet" media="all">
    <link href="vendor/font-awesome-4.7/css/font-awesome.min.css" rel="stylesheet" media="all">
    <!-- Font special for pages-->
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i,800,800i" rel="stylesheet">

    <!-- Vendor CSS-->
    <link href="vendor/select2/select2.min.css" rel="stylesheet" media="all">


    <!-- Main CSS-->
    <link href="css/main.css" rel="stylesheet" media="all">
    <style>
        .collegepic{
            width:100%;
        }
        h1{
            text-align: center;
        }
        ul.arrow{
            list-style-type:U+021E8;
        }
    </style>
</head>

<body>

    <div class="page-wrapper bg-gra-03 p-t-45 p-b-50">
        <div class="wrapper wrapper--w790">
            <?php include "header.html"; ?>            
            <?php include "tabs.html"; ?>
            <div class="card card-5">
                <div class="card-heading">
                    <h2 class="title">Courses Offered</h2>
                </div>
                <div class="card-body">                                        
		<h2>UG Courses</h2>
                        <ul>
                            <li>B.E Computer Science Engineering (NBA Accredited)</li>
                            <li>B.E Electronics and Communication Engineering (NBA Accredited)</li>
                            <li>B.Tech Information Technology</li>                            
                            <li>B.Tech Bio Technology (NBA Accredited)</li>
                            <li>B.E Electrical and Electronics Engineering </li>
                            <li>B.E Mechanical Engineering (NBA Accredited)</li>
                            <li>B.E Civil Engineering</li>
                            <li>B.E Mechatronics Engineering</li>
                            <li>B.Tech Artificial Intelligence and Data Science</li>                            
                        </ul><br>
                    <h2>PG Courses</h2>
                        <ul class ="arrow">
                            <li>M.Tech Polymer Science & Engineering</li>
                            <li>M.Tech Biotechnlogy</li>
                            <li>M.E Communication & Networking</li>
			    <li>M.E Computer Science and Engineering<sup><font color="blue">*</font></sup></li>
                            <li>M.E Manufacturing Engineering</li>
                            <li>M.E Power Systems Engineering</li>
                            <li>M.E Structural Engineering</li>
                            <font color="blue"><sup>*</sup> Subject to the approval of AICTE</font>
                        </ul><br>
                    
                    <h2>Research Centres</h2>
                        <ul>
                            <li>Polymer Technology</li>
                            <li>Biotechnology</li>
                            <li>Electrical and Electronics Engineering</li>
                            <li>Chemistry</li>
                            <li>Physics</li>
                            <li>Electronics and Communication Engineering</li>
                            <li>Mechanical Engineering</li>
                        </ul><br>
                    <div>
                        <img class="collegepic" src="images\banner_gallery.jpg">
                    </div>
                </div>
                
            </div>
        </div>
        
        <!-- Jquery JS-->
        <script src="vendor/jquery/jquery.min.js"></script>
        <!-- Vendor JS-->
        <script src="vendor/select2/select2.min.js"></script>
        <script src="vendor/datepicker/moment.min.js"></script>
        <script src="vendor/datepicker/daterangepicker.js"></script>

        <!-- Main JS-->
        <script src="js/global.js"></script>

</body>


</html>
<?php include 'Foot.php'; ?>                    
<!-- end document-->